(function( window, $, undefined ) {
	...
	$.elastislide.defaults 		= {
        ...
        slide       : function() {
            jQuery('.product .product-image-wrapper').unbind('mouseenter');
            jQuery(".preview").hide();
       }
    };
	...
	$.elastislide.prototype 	= {
		_init 				: function( options ) {
			...
			// <ul>
			this.$slider		= this.$el.find('div.carousel_items');

			// <li>
			this.$items			= this.$slider.children('div.carousel_item')
		...
		_toggleControls		: function( dir, status ) {
			...
			if( dir && status ) {
				if( status === 1 )
					...
					( dir === 'right' ) ? (this.$navNext.css({"cursor":"auto", "opacity":"1"}), this.$navNext.addClass('disable')) : (this.$navPrev.css({"cursor":"auto", "opacity":"1"}),this.$navPrev.addClass('disable'));
			}

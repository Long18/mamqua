...
var productCategory = {
    init: function () {
        ...
        productCategory.checkAll();
        productCategory.deleteMul();
        productCategory.deleteProduct();
        productCategory.deleteProductDb();
        productCategory.removeDeleteMul();
        productCategory.DeleteMulDb();
        productCategory.removeDeleteUser();
        productCategory.registerEvent();
        productCategory.ajaxStart();
    },
    saveData: function () {
    ...
    registerEvent: function () {
        ...
        $('#btn-Add-data').off('click').on('click', function (e) {
            e.preventDefault();
            productCategory.saveData();
        });
        $('.btn-Edit').off('click').on('click', function () {

            $('#myModalNorm').modal('show');
            var id = $(this).data('id');
            productCategory.viewDetail(id);
        });
    },
    viewDetail: function (id) {

        $.ajax({
            url: '/Admin/ProductCategory/ViewDetail',
            data: { id: id },
            type: 'POST',
            dataType: 'JSON',
            success: function (response) {
    ...
    deleteProduct: function () {
        ...
        $('.btn-remove-delete-db').off('click').on('click', function (e) {
            ...
            alertify.confirm("Xóa", "Bạn có chắc xóa", function () {//show thông báo nếu nhấn yes chạy hàm dưới
                $.ajax({//khởi tạo ajax
                    url: '/Admin/ProductCategory/Delete',//url nhận data
        ...
        $('#btn-deleteAll').off('click').on('click', function (e) {
            ...
            if (boxData == '') {//kiểm tra nếu mảng rỗng thì show thông báo
                ...
                alertify.confirm("Xóa", "Mục này sẽ được đưa vào thùng rác", function () {
                    $.ajax({
                        url: '/Admin/ProductCategory/DeleteSelected',
                        dataType: 'json',
                        type: 'POST',
        ...
        $('#btn-remove-deleteAll').off('click').on('click', function (e) {
            ...
            if (boxData == '') {
                ...
                alertify.confirm("Hủy xóa", "Mục này sẽ được lấy lại", function () {
                    $.ajax({
                        url: '/Admin/ProductCategory/DeleteSelectedRecycelBin',
                        dataType: 'json',
                        type: 'Post',
        ...
        $('#btn-remove-deleteAll-db').off('click').on('click', function (e) {
            ...
            if (boxData == '') {
                ...
                alertify.confirm("Xóa", "Bạn có chắc muốn xóa", function () {
                    $.ajax({
                        url: '/Admin/ProductCategory/DeleteSelectedDb',
                        dataType: 'json',
                        type: 'POST',
                        data: { ids: boxData.join(',') },
                        success: function (respone) {
...
var slug = function (str) {
    ...
    var from = "ÁÄÂÀÃÅẬẮẰẶẴẲẤẨẦẬẪČÇĆĎÉĚËÈÊẼĔȆỂẾỀỆỄÍÌÎÏŇÑÓÖÒÔÕỐỒỘỖỔØŘŔŠŤÚŮÜÙÛÝŸŽáäâàãạåắằẳặẵấầậẫẩăčçćďéěëèẹêẽĕȇếềệễểíìîïňñóöòọôõốồỗổộøðơờớởợỡřŕšťúůüùụữựừửữûýÿžþÞĐđßÆa·/_,:;";
    var to = "AAAAAAAAAAAAAAAAACCCDEEEEEEEEEEEEEIIIINNOOOOOOOOOOORRSTUUUUUYYZaaaaaaaaaaaaaaaaaacccdeeeeeeeeeeeeeeiiiinnooooooooooooooooooorrstuuuuuuuuuuuyyzbBDdBAa------";
